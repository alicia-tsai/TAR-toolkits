...
def word_to_ints(df, vocab, num_classes):
    ...
    for i in range(len(df)):
        ...
        if "building" in df.iloc[i]['label'].lower():
            ...
            y_by_class[0].append([1.0, 0.0, 0.0, 0.0])
        elif "infrastructure" in df.iloc[i]['label'].lower():
            x_by_class[1].append(int_tokens)
            y_by_class[1].append([0.0, 1.0, 0.0, 0.0])
        elif "resilience" in df.iloc[i]['label'].lower():
            x_by_class[2].append(int_tokens)
            y_by_class[2].append([0.0, 0.0, 1.0, 0.0])
        else: 
            x_by_class[3].append(int_tokens) 
            y_by_class[3].append([0.0, 0.0, 0.0, 1.0]) 
...
# Note, cleaned and lemmatized columns now contain lists of words
    ...
    x_train_by_class, y_train_by_class = word_to_ints(df_train, vocab, 4) 
    x_test_by_class, y_test_by_class = word_to_ints(df_test, vocab, 4)

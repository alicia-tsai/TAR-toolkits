...
def run_GAN(): 
   # tf.contrib.rnn
    raw_x_test = np.load('numpyData/raw_x_test.npy', allow_pickle = True)
    raw_y_test = np.load('numpyData/raw_y_test.npy', allow_pickle = True)
...
def run_CNN(): 
    ...
    with tf.Session() as sess:
        ...
        print("x_ shape" + str(raw_x_test.shape)) 
        print("y shape" + str(raw_y_test.shape))  
        for batch_x, batch_y in get_test_batch(raw_x_test, y_test_dummy, batch_size):

...
def get_usgs(min_magnitude, alert, date):
  ...
  #  print(earthquakes) 
    if VERBOSE: print("{0} {1} earthquake today.".format(len(earthquakes), alert))
...
def log_earthquakes(earthquakes, log_file):
    events = []
...
def get_earthquake_data(query_date, twitter_keys, news_key):
    ...
    log_path = os.path.join(os.getcwd(), DATA_DIR) 
    ...
    for (rupture_date, country) in earthquake_tweet.keys():
        tweet_file = os.path.join(os.getcwd(), DATA_DIR, "twitter",
                                  "{}_{}_earthquake.csv".format(rupture_date, country))
        news_file = os.path.join(os.getcwd(), DATA_DIR, "news",
                                 "{}_{}_earthquake.csv".format(rupture_date, country))
